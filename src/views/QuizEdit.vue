...
- 冒頭は見出しタグなしの <p> で始め、正解を自然な文脈で示しつつ要点を1〜2文で簡潔にまとめてください（UIに正解が表示されるため「正解は○○です」のような単調な書き出しは避け、「○○が正解ですが、ポイントは〜」「この問題の鍵は○○です」など毎回書き出しを変えてください）
- その後、トピックの性質に合わせて以下のセクション候補から3〜4個を選び、<h3> 見出し付きで構成してください。毎回すべてを使う必要はなく、順序もトピックに最適な流れで自由に並べ替えてください:
  ・不正解の選択肢との比較 — 正解の理由と他の選択肢が不正解である理由を具体的に。「○○ △△ 違い」で検索されそうな比較フレーズを含める
  ・仕組みを理解する / 内部動作 — 基礎概念・動作原理・処理フローなど技術的な裏側
  ・歴史的背景・なぜこうなったか — 仕様の経緯、バージョン間の変遷など
  ・よくあるミスと落とし穴 — 実際のエラーメッセージやアンチパターンをコード付きで紹介
  ・実務での活用シーン — 現場でどう使うか、ベストプラクティスやユースケース
  ・パフォーマンスと最適化 — 速度・メモリ・レンダリングなどの観点
  ・関連する技術・代替手段 — 類似API・ライブラリ・手法との比較
  ・セキュリティ上の注意点 — 脆弱性やサニタイズなど安全面
  ・デバッグのコツ — 問題が起きたときの調査・解決アプローチ
  ・知っておくと差がつくポイント — 上級者向けTipsや豆知識
- 見出しの文言は汎用的な定型句ではなくトピックに合わせて具体的にしてください（例: 「mapとforEachは何が違うのか」「クロージャが生まれる仕組み」「Reactの再レンダリング条件」など）
...
- コード例は説明の流れに合わせて適宜挿入し、<pre><code> タグで囲んでください`
}
